...
function ChatGPTSubmit()
	...
	local function receive_stream(_, data, _)
		...
		-- save the transcript but immediately discard to satisfy the acwrite buftype?
		-- vim.api.nvim_command("update /tmp/shellbot_tmp")
		-- os.remove("/tmp/shellbot_tmp")
	end
	...
	if job_id > 0 then
		...
		vim.api.nvim_buf_set_keymap(
			...
			"n",
			"<Enter>",
			":lua ChatGPTCancelCursorSync()<cr>",
...
function ChatGPTInit()
	...
	-- vim.api.nvim_command("autocmd BufWritePre <buffer> lua ChatGPTSubmit()")
